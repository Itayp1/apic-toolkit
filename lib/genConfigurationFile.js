...
const genConfigurationFile = async () => {
  ...
  const conf = {
    configuration: {
      ...
      env: { dev: { space: false, catalog: 'catalogName', organization: 'organizationName', server: 'Exampleserver.com', apiKey: 'apiKey' } },
    },
  };
  ...
  if (fillConfiguration === 'Yes') {
    ...
    conf.configuration = {
      ...
      env: { dev: { space: space != '' ? space : false, catalog: catalog, organization: organization, server: server, apiKey: apiKey } },
    };
  }
