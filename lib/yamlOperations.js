...
const { bad_format_msg, notFound_msg, general_msg } = require('./messages');
...
function yaml2Scripts(fileName) {
  ...
  const xslList = [];
  const operations = {
    gatewayscript: (obj, jsName) => gwsList.push(jsName) && fs.writeFileSync(`./${jsName}.js`, obj.gatewayscript.source, 'utf8'),
    xslt: (obj, xslName) => xslList.push(jsName) && fs.writeFileSync(`./${xslName}.xsl`, obj.xslt.source, 'utf8'),
  };
  try {
    const scripts2YamlOperation = new YamlOperation(operations);
    let fileContents = fs.readFileSync(`./${fileName}`, 'utf8');
    ...
    const DuplicatesGw = gwsList.filter((item, index) => gwsList.indexOf(item) != index);
    const DuplicatesXsl = xslList.filter((item, index) => xslList.indexOf(item) != index);

    if (DuplicatesGw.length > 0 || DuplicatesXsl.length > 0) {
      general_msg(`Duplicates file names detected please replace the filename into uniq filename ${DuplicatesGw.length > 0 ? `gwFiles:${DuplicatesGw}` : ''}  ${DuplicatesXsl.length > 0 ? `gwfiles:${DuplicatesXsl}` : ''}`);
    }
  } catch (e) {
    bad_format_msg(e.message);
    process.exit(0);
