...
const { BADFORMAT, GOODBYE, NOTFOUND, OPERATION_NOTFOUND, NOTFOUNDOPERATION, NOTFOUND_CONF, FINISHED, APIC_WAIT, PUBLISHING_ERR, CONF_ALREADY_EXIST, CANNOT_FIND_ENV } = require('./messages.json');

module.exports = {
  bad_format_msg: (msg) => console.log(chalk.redBright(BADFORMAT) + '\n' + chalk.redBright(msg)),
  goodbye_msg: () => console.log(chalk.blue(GOODBYE)),
  notFound_msg: (msg = '') => console.log(chalk.redBright(NOTFOUND + `\n` + msg)),
  ...
  cant_find_operation: (operation) => console.log(chalk.redBright(NOTFOUNDOPERATION + '\n' + operation)),
  general_msg: (msg = 'none') => console.log(chalk.blue(msg)),
};
