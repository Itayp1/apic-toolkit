...
const publish = async (enviorment) => {
  ...
  const { server, organization, catalog, stage } = env[enviorment];
  try {
    ...
    const publishCommand = `apic drafts:publish ${product.name}:${product.version}  --catalog ${catalog} --organization ${organization} --server=${server}      ${stage ? `--scope space --space ${stage}` : ''}`;
